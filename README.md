...
--m = path to metadata file
	- the metadata file should include a tab-separated line for each sample, formatted similarly to sampledata-to-qc.txt. Each line should contain the past to the forward fastq, path to the reverse fastq, path to output directory for QC results, group identifier, and sample number. They should be listed in that exact order. 
...
--i = path to genome index generated by STAR
	- if "auto" is specified, STAR will generate the genome index for you, but this can take 1-2 hours. To use a pre-generated index: use /home/project6/genome_index_new OR download the files from the following link: <<link>> . You will need to place all index files in one directory, then specify the name of this directory next to "-i" in your command. 
